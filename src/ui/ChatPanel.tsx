...
import { ChatMessage, MagiRenderState, MagiUnitId, MagiVerdict, OrchestratorMode } from "../types";
...
type ChatPanelProps = {
  ...
  voiceDictationStatus?: "idle" | "recording" | "transcribing";
  voicePlaybackMessageId?: string | null;
  voiceError?: string | null;
  onToggleVoiceDictation?: () => void;
  onPlayMessageTts?: (messageId: string, text: string) => void;
};
    ...
    <div className={`chat-shell ${props.fullscreen ? "chat-shell-fullscreen" : ""}`} data-tutorial-id="chat-shell">
      ...
      <div className={`chat-thread ${props.fullscreen ? "chat-thread-fullscreen" : ""}`}>
        ...
        {props.history.map((m, index) => {
          ...
          const canPlayVoice = (m.role === "user" || m.role === "assistant") && hasVisibleContent && !m.isStreaming;
          const voicePlaying = props.voicePlaybackMessageId === m.id;
              ...
              <div className={`chat-bubble-wrap ${m.role === "user" ? "from-user" : "from-agent"}`}>
                <div className="chat-meta">
                  ...
                  {canPlayVoice ? (
                    <button
                      type="button"
                      className={`chat-voice-play-btn ${voicePlaying ? "active" : ""}`}
                      onClick={() => props.onPlayMessageTts?.(m.id, renderedContent)}
                      title={voicePlaying ? "停止朗讀" : "朗讀這則訊息"}
                      aria-label={voicePlaying ? "停止朗讀" : "朗讀這則訊息"}
                    >
                      {voicePlaying ? <StopIcon /> : <SpeakerIcon />}
                    </button>
                  ) : null}
                </div>
      ...
      <div className={`chat-composer ${props.fullscreen ? "chat-composer-fullscreen" : ""}`}>
        ...
        <button
          ...
          className={`chat-voice-btn ${props.voiceDictationStatus === "recording" ? "recording" : ""}`}
          disabled={props.voiceDictationStatus === "transcribing" || !props.onToggleVoiceDictation}
          title={
            props.voiceDictationStatus === "recording"
              ? "停止錄音並轉寫"
              : props.voiceDictationStatus === "transcribing"
              ? "轉寫中"
              : "語音輸入"
          }
          aria-label={
        ...
        >
          {props.voiceDictationStatus === "recording" ? <StopIcon /> : <MicIcon />}
        </button>
        <button
      ...
      {props.voiceError ? <div className="chat-voice-error">{props.voiceError}</div> : null}
    </div>
  );
}
...
function MicIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true">
      ...
      <path d="M3.2 7.2a4.8 4.8 0 0 0 9.6 0M8 12v2.2M5.8 14.2h4.4" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" />
    </svg>
  );
}
...
function SpeakerIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true">
      <path d="M2.5 6.2v3.6h2.2L8 12.5v-9L4.7 6.2H2.5Z" stroke="currentColor" strokeWidth="1.35" strokeLinejoin="round" />
      <path d="M10.2 5.4a3 3 0 0 1 0 5.2M11.8 3.8a5.2 5.2 0 0 1 0 8.4" stroke="currentColor" strokeWidth="1.25" strokeLinecap="round" />
    </svg>
  );
}
