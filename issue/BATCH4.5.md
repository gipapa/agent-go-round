# Batch 4.5 — Reliability / Resource Limits（3-5 天）
...
## 後續鋪墊
- Batch 4 拆 App.tsx 時，`useOneToOne` / `useSkillExecution` hook 內持有 `AbortController`，可直接呼叫此 batch 已備好的 deadline helper
- Issue 14 的 race condition 解法（execution lock）會直接用到 abort 機制（取消舊 instance）

## 不要做的事
- 不要在這個 batch 同時動「拆 App.tsx」（Batch 4 工作）
