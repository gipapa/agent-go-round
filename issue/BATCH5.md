# Batch 5 — 大型重構：App.tsx 拆解 + Storage 加固 + 測試補強（2-4 週）
...
- **Batch 4 強烈建議先完成**：deadline / abort helper 與 user code timeout 已就緒，拆出 hook 時可直接整合
...
2. **Trace per-execution**
   ...
   - 中途取消（Batch 4 的 deadline）只會丟掉那個 local trace，不污染其他執行流
